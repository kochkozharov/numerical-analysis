...
def main():
    ...
    if number == 1:
        ...
        for Xi in [ [0.1, 0.5, 0.9, 1.3], [0.1, 0.5, 1.1, 1.3] ]:
            Yi = [f(x) for x in Xi]
            # Лагранж
            P_L, term_expr_L = lagrange_poly(Xi, Yi)
            # Ньютон
            C_N, term_expr_N = newton_coeffs(Xi, Yi)
            ...
            print(f"Многочлен Лагранжа: {term_expr_L}")
            print(f"Многочлен Ньютона: {term_expr_N}\n")
            # График
        ...
        X_star = 2.0
        cf, term_expr_N = newton_coeffs(X, Y)
        
        print(f"Многочлен Ньютона в форме Ньютона:")
        print(term_expr_N)
        # Вывод коэффициентов в стандартной форме
        print("Коэффициенты многочлена Ньютона в обычной форме:")
        for i, c in enumerate(cf):
            print(f"  a{i} = {c:.6f}")
        
        # Многочлен в стандартной форме
        poly_str = " + ".join([f"{cf[i]:.6f} * x^{i}" for i in range(len(cf))])
        poly_str = poly_str.replace("x^0", "1").replace(" + -", " - ")
        print(f"P(x) = {poly_str}")
        ...
        # Вывод коэффициентов первой производной
        print("\nКоэффициенты первой производной:")
        for i, c in enumerate(d1):
            print(f"  a{i} = {c:.6f}")
        
        # Многочлен первой производной в стандартной форме
        if d1:
            d1_str = " + ".join([f"{d1[i]:.6f} * x^{i}" for i in range(len(d1))])
            d1_str = d1_str.replace("x^0", "1").replace(" + -", " - ")
            print(f"P'(x) = {d1_str}")
        
        # Вывод коэффициентов второй производной
        print("\nКоэффициенты второй производной:")
        for i, c in enumerate(d2):
            print(f"  a{i} = {c:.6f}")
        
        # Многочлен второй производной в стандартной форме
        if d2:
            d2_str = " + ".join([f"{d2[i]:.6f} * x^{i}" for i in range(len(d2))])
            d2_str = d2_str.replace("x^0", "1").replace(" + -", " - ")
            print(f"P''(x) = {d2_str}")
        
        print(f"\nЗначения в точке x* = {X_star}:")
        print(f"P({X_star})  = {P(X_star):.6f}")
        print(f"P'({X_star}) = {dP(X_star):.6f}")
        print(f"P''({X_star})= {ddP(X_star):.6f}")
        ...
        plt.subplot(3, 1, 1)
        plt.plot(xs, ys, label='Многочлен P(x)')
        plt.scatter(X, Y, c='red', label='Узлы')
        plt.scatter([X_star], [P(X_star)], c='green', label='x*')
        plt.legend()
        plt.title('Интерполяционный многочлен')
        
        # График первой производной
        plt.subplot(3, 1, 2)
        plt.plot(xs, ys1, label='P\'(x)')
        plt.scatter([X_star], [dP(X_star)], c='green')
        plt.legend()
        plt.title('Первая производная')
        
        # График второй производной
        plt.subplot(3, 1, 3)
        plt.plot(xs, ys2, label='P\'\'(x)')
        plt.scatter([X_star], [ddP(X_star)], c='green')
        plt.legend()
        plt.title('Вторая производная')
